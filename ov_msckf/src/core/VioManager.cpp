...
VioManager::VioManager(VioManagerOptions& params_) {
    ...
    // Create the state!!
    state = std::make_unique<State>(params.state_options);
    ...
    if(params.use_klt) {
        trackFEATS = std::make_unique<TrackKLT>(params.num_pts,state->_options.max_aruco_features,params.fast_threshold,params.grid_x,params.grid_y,params.min_px_dist);
        trackFEATS->set_calibration(params.camera_intrinsics, params.camera_fisheye);
    } else {
        trackFEATS = std::make_unique<TrackDescriptor>(params.num_pts,state->_options.max_aruco_features,params.fast_threshold,params.grid_x,params.grid_y,params.knn_ratio);
        trackFEATS->set_calibration(params.camera_intrinsics, params.camera_fisheye);
    }

    // Initialize our aruco tag extractor
    if(params.use_aruco) {
        trackARUCO = std::make_unique<TrackAruco>(state->_options.max_aruco_features, params.downsize_aruco);
        trackARUCO->set_calibration(params.camera_intrinsics, params.camera_fisheye);
    }

    // Initialize our state propagator
    propagator = std::make_unique<Propagator>(params.imu_noises, params.gravity);

    // Our state initialize
    initializer = std::make_unique<InertialInitializer>(params.gravity,params.init_window_time,params.init_imu_thresh);

    // Make the updater!
    updaterMSCKF = std::make_unique<UpdaterMSCKF>(params.msckf_options,params.featinit_options);
    updaterSLAM = std::make_unique<UpdaterSLAM>(params.slam_options,params.aruco_options,params.featinit_options);
}
...
void VioManager::feed_measurement_stereo(double timestamp, cv::Mat& img0, cv::Mat& img1, size_t cam_id0, size_t cam_id1) {
    ...
    if(params.use_stereo) {
        ...
        std::thread t_l = timed_thread(cts, "slam2 feed l", &TrackBase::feed_monocular, trackFEATS.get(), boost::ref(timestamp), boost::ref(img0), boost::ref(cam_id0));
        std::thread t_r = timed_thread(cts, "slam2 feed r", &TrackBase::feed_monocular, trackFEATS.get(), boost::ref(timestamp), boost::ref(img1), boost::ref(cam_id1));
        t_l.join();
        t_r.join();
    }
...
void VioManager::feed_measurement_simulation(double timestamp, const std::vector<int> &camids, const std::vector<std::vector<std::pair<size_t,Eigen::VectorXf>>> &feats) {
    ...
    if(trackSIM == nullptr) {
        ...
        trackFEATS = std::make_unique<TrackSIM>(state->_options.max_aruco_features);
        trackFEATS->set_calibration(params.camera_intrinsics, params.camera_fisheye);
        printf(RED "[SIM]: casting our tracker to a TrackSIM object!\n" RESET);
    }

    // Cast the tracker to our simulation tracker
    trackSIM = dynamic_cast<TrackSIM*>(trackFEATS.get());
