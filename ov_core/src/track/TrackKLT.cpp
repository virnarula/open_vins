...
void TrackKLT::feed_stereo(double timestamp, cv::Mat &img_leftin, cv::Mat &img_rightin, size_t cam_id_left, size_t cam_id_right) {
    ...
    //printf("[TIME-KLT]: %.4f ms for pyramid\n",(rT2-rT1).total_microseconds() * 1e-3);
    //printf("[TIME-KLT]: %.4f ms for detection\n",(rT3-rT2).total_microseconds() * 1e-3);
    //printf("[TIME-KLT]: %.4f ms for temporal klt\n",(rT4-rT3).total_microseconds() * 1e-3);
    //printf("[TIME-KLT]: %.4f ms for stereo klt\n",(rT5-rT4).total_microseconds() * 1e-3);
    //printf("[TIME-KLT]: %.4f ms for feature DB update (%d features)\n",(rT6-rT5).total_microseconds() * 1e-3, (int)good_left.size());
    //printf("[TIME-KLT]: %.4f ms for total\n",(rT6-rT1).total_microseconds() * 1e-3);
