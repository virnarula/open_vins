...
void TrackDescriptor::feed_stereo(double timestamp, cv::Mat &img_leftin, cv::Mat &img_rightin, size_t cam_id_left, size_t cam_id_right) {
    ...
    // Lets match temporally
    std::thread t_ll = timed_thread(cts, "slam2 rob_match l", &TrackDescriptor::robust_match, this, boost::ref(pts_last[cam_id_left]), boost::ref(pts_left_new),
                                       boost::ref(desc_last[cam_id_left]), boost::ref(desc_left_new), cam_id_left, cam_id_left, boost::ref(matches_ll));
    std::thread t_rr = timed_thread(cts, "slam2 rob_match r", &TrackDescriptor::robust_match, this, boost::ref(pts_last[cam_id_right]), boost::ref(pts_right_new),
...
void TrackDescriptor::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1,
    ...
    std::thread t_0 = timed_thread(cts, "slam2 grid l", &Grider_FAST::perform_griding, boost::cref(img0), boost::ref(pts0_ext),
                                      num_features, grid_x, grid_y, threshold, true);
    std::thread t_1 = timed_thread(cts, "slam2 grid r", &Grider_FAST::perform_griding, boost::cref(img1), boost::ref(pts1_ext),
                                      num_features, grid_x, grid_y, threshold, true);
    ...
    std::thread t_desc0 = timed_thread(cts, "slam2 orb l", [&]{this->orb0->compute(img0, pts0_ext, desc0_ext);});
    std::thread t_desc1 = timed_thread(cts,"slam2 orb r", [&]{this->orb1->compute(img1, pts1_ext, desc1_ext);});
