...
void UpdaterSLAM::update(State *state, std::vector<Feature*>& feature_vec) {
    ...
    while(it2 != feature_vec.end()) {
        ...
        if(chi2 > chi2_multipler*chi2_check) {
            if((int)feat.featid < state->options().max_aruco_features)
				std::cout << "[SLAM-UP]: rejecting aruco tag " << (int)feat.featid << " for chi2 thresh (" << chi2 << " > " << chi2_multipler*chi2_check << " )" << std::endl;
            (*it2)->to_delete = true;
            it2 = feature_vec.erase(it2);
            continue;
        }
    ...
    // Debug print timing information
	std::cout << "[SLAM-UP]: " << (rT1-rT0).total_microseconds() * 1e-6 << " seconds to clean" << std::endl;
	std::cout << "[SLAM-UP]: " << (rT2-rT1).total_microseconds() * 1e-6 << " seconds creating linear system" << std::endl;
	std::cout << "[SLAM-UP]: " << (rT3-rT2).total_microseconds() * 1e-6 << " seconds to update (" << (int)feature_vec.size() << " feats of " << (int)Hx_big.rows() << " size)" << std::endl;
	std::cout << "[SLAM-UP]: " << (rT3-rT1).total_microseconds() * 1e-6 << " seconds total" << std::endl;
