...
void TrackKLT::feed_monocular(double timestamp, cv::Mat &img, size_t cam_id) {
    ...
    if(mask_ll.empty()) {
		...
		std::cout << "[KLT-EXTRACTOR]: Failed to get enough points to do RANSAC, resetting....." << std::endl;
        return;
    }
...
void TrackKLT::feed_stereo(double timestamp, cv::Mat &img_leftin, cv::Mat &img_rightin, size_t cam_id_left, size_t cam_id_right) {
    ...
    perform_matching_stereo(imgpyr_left, imgpyr_right, pts_left_new, pts_right_new, cam_id_left, cam_id_right, mask_lr);
    rT5 =  boost::posix_time::microsec_clock::local_time();
    ...
    // Timing information
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_KLT_temporal_left/total_temporal_left_calls << "ms for KLT_temporal_left" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_undistort_temporal_left/total_temporal_left_calls << " ms for Undistort_temporal_left" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_RANSAC_temporal_left/total_temporal_left_calls << " ms for RANSAC_temporal_left" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_KLT_temporal_right/total_temporal_right_calls << " ms for KLT_temporal_right" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_undistort_temporal_right/total_temporal_right_calls << " ms for Undistort_temporal_right" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_RANSAC_temporal_right/total_temporal_right_calls << " ms for RANSAC_temporal_right" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_KLT_stereo/total_stereo_calls << " ms for KLT_stereo" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_undistort_stereo/total_stereo_calls <<" ms for Undistort_stereo" << std::endl;
	std::cout << "[AVERAGE-MATCHING-TIME]: " << total_RANSAC_stereo/total_stereo_calls << " ms for RANSAC_stereo" << std::endl;
	
    std::cout << "[TIME-KLT]: " << pyramid_time << " ms for pyramid" << std::endl;
    std::cout << "[TIME-KLT]: " << detection_time << " ms for detection" << std::endl;
	std::cout << "[TIME-KLT]: " << temporal_klt_time << " ms for temporal klt" << std::endl;
    std::cout << "[TIME-KLT]: " << stereo_klt_time << " ms for stereo klt" << std::endl;
    std::cout << "[TIME-KLT]: " << feature_db_update_time << " ms for feature DB update (%d features)" << std::endl;
    std::cout << "[TIME-KLT]: " << total_feed_stereo_time << " ms for total" << std::endl;

    std::cout << "[AVERAGE-KLT]: " << total_pyramid_time/feed_stereo_count << " ms for pyramid" << std::endl;
	std::cout << "[AVERAGE-KLT]: " << total_detection_time/feed_stereo_count << " ms for detection" << std::endl;
	std::cout << "[AVERAGE-KLT]: " << total_temporal_klt_time/feed_stereo_count << " ms for temporal klt" << std::endl;
    std::cout << "[AVERAGE-KLT]: " << total_stereo_klt_time/feed_stereo_count << " ms for stereo klt" << std::endl;
    std::cout << "[AVERAGE-KLT]: " << total_feature_db_update_time/feed_stereo_count << " ms for feature DB update (%d features)" << std::endl;
    std::cout << "[AVERAGE-KLT]: " << total_total_feed_stereo_time/feed_stereo_count << " ms for total" << std::endl;
}
...
void TrackKLT::perform_detection_stereo(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr,
    ...
    std::cout << "[TIME-DETECTION]: " << (rTDetect2 - rTDetect1).total_microseconds() * 1e-3 <<" ms for Grid" << std::endl;
std::cout << "[TIME-DETECTION]: " << (rTDetect3 - rTDetect2).total_microseconds() * 1e-3 << " ms for KLT" << std::endl;
}
...
void TrackKLT::perform_matching(const std::vector<cv::Mat>& img0pyr, const std::vector<cv::Mat>& img1pyr,
	...
	std::cout << "[TIME-MATCHING]: " << KLT_time << " ms for KLT_temporal" << std::endl;
	std::cout << "[TIME-MATCHING]: " << undistort_time << " ms for Undistort_temporal" << std::endl;
    std::cout << "[TIME-MATCHING]: " << RANSAC_time << "ms for RANSAC_temporal" << std::endl;
...
void TrackKLT::perform_matching_left(const std::vector<cv::Mat>& img0pyr, const std::vector<cv::Mat>& img1pyr,
	...
	std::cout << "[TIME-MATCHING]: " << KLT_time << " ms for KLT_temporal_left" << std::endl;
	std::cout << "[TIME-MATCHING]: " << undistort_time << " ms for Undistort_temporal_left" << std::endl;
    std::cout << "[TIME-MATCHING]: " << RANSAC_time << "ms for RANSAC_temporal_left" << std::endl;
...
void TrackKLT::perform_matching_right(const std::vector<cv::Mat>& img0pyr, const std::vector<cv::Mat>& img1pyr,
	...
	std::cout << "[TIME-MATCHING]: " << KLT_time << " ms for KLT_temporal_right" << std::endl;
	std::cout << "[TIME-MATCHING]: " << undistort_time << " ms for Undistort_temporal_right" << std::endl;
    std::cout << "[TIME-MATCHING]: " << RANSAC_time << "ms for RANSAC_temporal_right" << std::endl;
}
...
void TrackKLT::perform_matching_stereo(const std::vector<cv::Mat>& img0pyr, const std::vector<cv::Mat>& img1pyr,
	...
	total_KLT_stereo += KLT_time;
	total_undistort_stereo += undistort_time;
	total_RANSAC_stereo += RANSAC_time;

	total_stereo_calls++;
	
    std::cout << "[TIME-MATCHING]: " << KLT_time << " ms for KLT_stereo" << std::endl;
	std::cout << "[TIME-MATCHING]: " << undistort_time << " ms for Undistort_stereo" << std::endl;
    std::cout << "[TIME-MATCHING]: " << RANSAC_time << "ms for RANSAC_stereo" << std::endl;
}
